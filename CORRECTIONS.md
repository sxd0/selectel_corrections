...
### 1. приложение не читает DATABASE_URL из .env
...
### 2. дефолтный DSN указывает на несуществующую БД
...
### 3. парсинг запускается каждые 5 секунд, а должен каждые 5 минут
...
### 4. парсер падает, если city = null
...
### 6. фильтр по городу в CRUD называется не так, как поле
...
### 7. при дубле external_id возвращается 200 и “неправильный” ответ
...
### 8. external_id допускает NULL, из-за этого ломается уникальность и логика upsert
